...
from file_map_dictionary   import compare_new_old_dimuon_file_map
...
if __name__ == "__main__":
  ...
  final_state_mode = "dimuon" # default mutau [possible values ditau, mutau, etau, dimuon, emu]
  ...
  file_map = compare_new_old_dimuon_file_map
  ...
  for process in file_map: 
    ...
    if (process=="DataOldDimuon"):
      using_directory = "/Volumes/IDrive/HTauTau_Data/skims/dimuon_2022postEE/DataReReco/"
      useMiniIso = False
    if (process=="DataNewDimuon"):
      using_directory = "/Users/ballmond/LocalDesktop/HiggsTauTau/Run3Unskimmed/MuonMiniIso/"
      useMiniIso = True
  ...
  data_dict_old = {"DataOldDimuon" : data_dictionary["DataOldDimuon"]}
  data_dict_new = {"DataNewDimuon" : data_dictionary["DataNewDimuon"]}
  ...
  for var in vars_to_plot:
    ...
    h_data_old = get_binned_data(data_dict_old, var, xbins, lumi)
    h_data_new = get_binned_data(data_dict_new, var, xbins, lumi)

    # plot everything :)
    plot_data(hist_ax, xbins, h_data_old, lumi, color="red",  label="2022 Era G Old", fillstyle="none")
    plot_data(hist_ax, xbins, h_data_new, lumi, color="blue", label="2022 Era G New", marker="x")

    make_ratio_plot(hist_ratio, xbins, h_data_old, h_data_new)
